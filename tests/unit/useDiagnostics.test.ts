...
describe('useDiagnostics', () => {
  ...
  describe('description diagnostics', () => {
    ...
    it('returns green status when description is optimal length', () => {
      const tags = createMetaTags({ 
        description: 'This is a good description that provides enough context about the page content for search engines and users.' 
      })
      const result = generateDiagnostics(tags)
  ...
  describe('OG tags diagnostics', () => {
    ...
    it('returns yellow status when some OG tags are missing', () => {
      const tags = createMetaTags({
        og: {
          title: 'OG Title',
          description: undefined,
  ...
  describe('OG image diagnostics', () => {
    it('returns yellow status when og:image is missing', () => {
      const tags = createMetaTags()
      const result = generateDiagnostics(tags)
  ...
  describe('Twitter Card diagnostics', () => {
    it('returns yellow when OG tags exist but twitter:card is missing', () => {
      const tags = createMetaTags({
        og: {
          title: 'OG Title',
          description: 'Description',
      ...
      const result = generateDiagnostics(tags)
      expect(result.twitterCard.status).toBe('yellow')
  ...
  describe('canonical diagnostics', () => {
    it('returns yellow when canonical is missing', () => {
      const tags = createMetaTags()
      const result = generateDiagnostics(tags)
      expect(result.canonical.status).toBe('yellow')
  ...
  describe('overall status', () => {
    ...
    it('returns yellow overall when worst is yellow', () => {
      const tags = createMetaTags({
        title: 'Good Title',
        description: 'A description that is long enough to pass the minimum check',
        og: {
          title: 'OG Title',
          description: 'OG Description',
          type: undefined,
    ...
    it('returns green overall when all checks pass', () => {
      const tags = createMetaTags({
        title: 'Good Title',
        description: 'A description that is long enough to pass the minimum check for search engines',
