...
> **Note:** The CLI lives inside the [icjia-metapeek](https://github.com/ICJIA/icjia-metapeek) monorepo at `packages/cli/`. The previous standalone `icjia-metapeek-cli` repository has been archived in favor of this single source of truth.

**Web app:** https://metapeek.icjia.app

**Monorepo:** https://github.com/ICJIA/icjia-metapeek
...
cd icjia-metapeek/packages/cli
chmod +x metapeek
...
```bash
git clone https://github.com/ICJIA/icjia-metapeek.git
ln -s "$(pwd)/icjia-metapeek/packages/cli/metapeek" /usr/local/bin/metapeek
```

### Option 3: Direct download

```bash
curl -fsSL https://raw.githubusercontent.com/ICJIA/icjia-metapeek/main/packages/cli/metapeek -o /usr/local/bin/metapeek && chmod +x /usr/local/bin/metapeek
```
...
```bash
sudo ln -s "$(pwd)/icjia-metapeek/packages/cli/metapeek" /usr/local/bin/metapeek
```
...
```bash
mkdir -p ~/.local/bin
ln -s "$(pwd)/icjia-metapeek/packages/cli/metapeek" ~/.local/bin/metapeek
```
